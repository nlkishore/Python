...
def connect_and_execute():
    ...
    if not market or not command_key:
        messagebox.showerror("Error", "Please select a market and a command.")
        return

    if market not in server_config or market not in commands_config:
        messagebox.showerror("Error", f"Market '{market}' not found in configuration.")
        return

    if command_key not in commands_config[market]:
        messagebox.showerror("Error", f"Command '{command_key}' not found for '{market}'.")
        return
...
# Command Selection
ttk.Label(root, text="Select Command:").pack()
