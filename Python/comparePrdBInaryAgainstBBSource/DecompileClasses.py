...
def decompile_class_files(input_dir, output_dir, decompiler):
    ...
    for root, _, files in os.walk(input_dir):
        for file in files:
            if file.endswith(".class"):
                ...
                if decompiler == "javap":
                    subprocess.run(["javap", "-c", "-p", "-s", "-v", "-l", "--constants", class_file_path], stdout=open(java_file_path, 'w'))
                elif decompiler == "jd-cli":
                    subprocess.run(["jd-cli", class_file_path, "-od", os.path.dirname(java_file_path)])
                elif decompiler == "jd-gui":
                    subprocess.run(["jd-gui", class_file_path, "-od", os.path.dirname(java_file_path)])
                else:
...
# Decompile the class files using the specified option
decompile_class_files(input_directory, output_directory, decompiler_option)
