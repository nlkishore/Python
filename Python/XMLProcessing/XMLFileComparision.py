'''Compare XML files irrespecive of Element values are different . element values differnce may be falgged out'''
...
def compare_xml(file1, file2):
    ...
    common_elements = {k: elements1[k] for k in elements1 if k in elements2 and elements1[k] == elements2[k]}
    ...
    return common_elements, missing_in_file1, missing_in_file2

def create_excel_report(common, missing1, missing2, output_file):
    df_common = pd.DataFrame(list(common.items()), columns=['Element', 'Value'])
